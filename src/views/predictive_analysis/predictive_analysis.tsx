...
const PredictiveAnalytics = () => {
    ...
    const handleDownloadExcel = () => {
        ...
        header.forEach((col, idx) => {
            ...
            worksheet[cellAddress].s = {
                fill: { fgColor: { rgb: "D9EDE3" } }, // Fondo #D9EDE3 (azul/claro de marca)
    ...
    const extractActionsSection = (html: string, label: string) => {
        if (!html) return "";
        // Escapar caracteres especiales del label para usar en RegExp
        const escaped = label.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        // Busca desde <b>Label:</b> hasta el siguiente bloque <b>Controllable by ...</b> o el final
    ...
    // Badge lindo para nivel salarial
    const renderSalaryBadge = (level: string | null) => {
        ...
        if (isLow) {
            bgFrom = "#FDE7E9"; bgTo = "#FAD1D5"; text = "#7A0A1A"; border = "#F5B7BF"; icon = "⬇️";
        } else if (isCompetitive) {
            bgFrom = "#FFF7E0"; bgTo = "#FFE8B0"; text = "#8A5B00"; border = "#FFD777"; icon = "⚖️";
        } else if (isHigh) {
            bgFrom = "#E6F4EA"; bgTo = "#D1EFE0"; text = "#0D4B3B"; border = "#B7E1C9"; icon = "⬆️";
        }
                ...
                {selectedEmployee && (
                    <>
                        ...
                        <DialogContent dividers>
                            {/* Distribución a dos columnas como el mockup */}
                            <Box sx={{ display: "flex", gap: 2, mb: 2, flexDirection: { xs: "column", md: "row" } }}>
                                {/* Columna izquierda: Employee Overview */}
                                <Box sx={{ flex: 1 }}>
                                    ...
                                    >
                                        ...
                                        <Box>
                                            <Box sx={{ display: "flex", alignItems: "center", gap: 1.25, mb: 0.5, flexWrap: "wrap" }}>
                                                <Typography variant="body1" fontWeight={800} sx={{ m: 0 }}>
                                                    Compensation Level:
                                                </Typography>
                                                {renderSalaryBadge(employeeSalaryLevel)}
                                            </Box>
                                            <Typography variant="body2" sx={{ color: "text.secondary" }}>
                                                Based on current local market benchmark data
                                            </Typography>
                                        </Box>
                                    </Box>

                                    {/* Card: Prioritized Risk Drivers */}
