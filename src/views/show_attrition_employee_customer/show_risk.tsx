        ...
        fetch(`${config.rutaApi}show_risks`, requestOptions)
            ...
            .then((result) => {
                ...
                const formattedData: showRisks[] = data.map((item: any) => ({
                    // Aseguramos usar el id real del empleado para consultar employees_profile
                    id: item.fkid_employe || item.id || 0,
        ...
        try {
            ...
            fetch(`${config.rutaApi}employees_profile`, requestOptions)
                ...
                .then((result) => {
                    ...
                    const emp = Array.isArray(dataEmp) ? dataEmp[0] : dataEmp;
                    if (emp) {
                        const sl = emp.salary_level ?? emp.salaryLevel ?? null;
